(function() {
  ...
  function updateHeader() {
    ...
    if (scrolled) {
      ...
      if(relay)  relay.style.color  = 'rgba(245,245,240,.4)';
      if(line)   line.style.background = 'rgba(245,245,240,.15)';
      links.forEach(a => {
        const isActive = a.getAttribute('href') === page;
        a.style.color = isActive ? '#f5f5f0' : 'rgba(245,245,240,.5)';
        a.style.borderBottomColor = isActive ? 'rgba(245,245,240,.3)' : 'transparent';
      });
    } else {
      hdr.style.background = 'rgba(0,0,0,0)';
      hdr.style.borderBottom = 'none';
      hdr.style.backdropFilter = 'none';
      if(motion) motion.style.color = '#f9f8f6';
      if(relay)  relay.style.color  = 'rgba(255,255,255,.6)';
      if(line)   line.style.background = 'rgba(255,255,255,.25)';
      links.forEach(a => {
        const isActive = a.getAttribute('href') === page;
  ...
  links.forEach(a => {
    a.addEventListener('mouseenter', () => {
      a.style.color = '#f5f5f0';
      a.style.borderBottomColor = '#f5f5f0';
    });
